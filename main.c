 ...
 * cscript
 * By: biodag
 * A modification of c4, "C in 4 functions"
 */
...
int assembly;	// print assembly code
int token; 		// current token

// instructions for vm runtime
...
// tokens and classes (from c4, nothing here to improve)
enum {
...
int basetype;					// decl type
int expr_type;  				// expr type
int index_of_bp; 				// bp index on pointer frame
...
int main(int argc, char **argv) {
	...
	if (!(text = malloc(poolsize))) {
		printf("Not enough malloc (%d) for text area!\n", poolsize);
		return -1;
	}
	if (!(data = malloc(poolsize))) {
		printf("Not enough malloc (%d) for data area\n", poolsize);
		return -1;
	}
	if (!(stack = malloc(poolsize))) {
		printf("Not enough malloc (%d) for stack area\n", poolsize);
		return -1;
	}
	if (!(symbols = malloc(poolsize))) {
		printf("Not enough malloc (%d) for symbol table\n", poolsize);
		return -1;
	}
